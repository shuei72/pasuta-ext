...
async function buildHighlightedRenderData(lines, options) {
  ...
  const { codeToTokens, bundledLanguages, bundledLanguagesAlias } = await shikiImport;
  const language = resolveShikiLanguage(options.languageId, bundledLanguages, bundledLanguagesAlias);
...
function getLineNumberColor(theme) {
  return theme === "light-plus" ? "#237893" : "#858585";
}
